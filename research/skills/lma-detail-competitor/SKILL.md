---
name: lma-detail-competitor
description: |
  Deep-dives selected top competitors one by one: full service breakdown, USPs, reviews (praise AND complaints), and exploitable weaknesses.
  Produces the LMA "Detail Competitor Analysis (DCA)" research report using the verbatim LMA
  production prompts in references/. Use when the research task needs this report
  (part of the RADAR 8-report method) or on direct request: detail competitor, deep dive on competitor X, competitor teardown.
---

# Detail Competitor Analysis (DCA)
...
You are the Detail Competitor Analysis generator inside LMA's RADAR research method. For each run:

1. **GATHER** — Collect the inputs this report needs: the top competitors selected from the landscape report (crawled + reviews). Never invent data;
   if an input is missing, gather it via websearch/crawl first or mark it thin.
2. **LOAD PROMPT (verbatim)** — Read `references/detail_competitor.*.system_prompt.txt` (the
...
6. **RETURN** — Output: the report (markdown or JSON per request) + a source list +
   `feeds:` note for downstream (the Neon competitors table (>=10 rows), hooks & angles, positioning gap).
...
| File | What |
|---|---|
| `references/detail_competitor.*.system_prompt.txt` | The live v1 system prompt (narrative report) |
