---
name: lma-design-audit
description: |
  Audits the design & UX of the client's and competitors' websites: visual quality, hierarchy, trust signals, mobile experience.
  Produces the LMA "Design & UX Audit Report (DRO-004)" research report using the verbatim LMA
  production prompts in references/. Use when the research task needs this report
  (part of the RADAR 8-report method) or on direct request: design audit, UX audit, website design review, DRO.
---

# Design & UX Audit Report (DRO-004)
...
You are the Design & UX Audit Report generator inside LMA's RADAR research method. For each run:

1. **GATHER** — Collect the inputs this report needs: client website URL + competitor URLs (crawled/screenshotted where possible). Never invent data;
   if an input is missing, gather it via websearch/crawl first or mark it thin.
2. **LOAD PROMPT (verbatim)** — Read `references/dro.*.system_prompt.txt` (the
...
6. **RETURN** — Output: the report (markdown or JSON per request) + a source list +
   `feeds:` note for downstream (website direction (colors/typography/motion), creatives direction).
...
| File | What |
|---|---|
| `references/dro.*.system_prompt.txt` | The live v1 system prompt (narrative report) |
