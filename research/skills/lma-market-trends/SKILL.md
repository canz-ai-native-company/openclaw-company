---
name: lma-market-trends
description: |
  Captures where this niche market is heading: demand signals, rising/declining services, seasonal patterns, and what that means for the client.
  Produces the LMA "Market Trends Report (MTR-007)" research report using the verbatim LMA
  production prompts in references/. Use when the research task needs this report
  (part of the RADAR 8-report method) or on direct request: market trends, demand, is this market growing, MTR.
---

# Market Trends Report (MTR-007)
...
You are the Market Trends Report generator inside LMA's RADAR research method. For each run:

1. **GATHER** — Collect the inputs this report needs: websearch (trend/demand sources for niche + location). Never invent data;
   if an input is missing, gather it via websearch/crawl first or mark it thin.
2. **LOAD PROMPT (verbatim)** — Read `references/market_trends.*.system_prompt.txt` (the
...
6. **RETURN** — Output: the report (markdown or JSON per request) + a source list +
   `feeds:` note for downstream (strategic-positioning, campaign timing).
...
| File | What |
|---|---|
| `references/market_trends.*.system_prompt.txt` | The live v1 system prompt (narrative report) |
