---
name: lma-competitive-landscape
description: |
  Maps the client's real competitive field: who the actual competitors are, how they position, market structure, and where the client stands.
  Produces the LMA "Competitive Landscape Report (CLR-001)" research report using the verbatim LMA
  production prompts in references/. Use when the research task needs this report
  (part of the RADAR 8-report method) or on direct request: competitive landscape, who are the competitors, market landscape, CLR.
---

# Competitive Landscape Report (CLR-001)
...
You are the Competitive Landscape Report generator inside LMA's RADAR research method. For each run:

1. **GATHER** — Collect the inputs this report needs: client (business_name, niche, location), competitor list/URLs from websearch, crawled competitor pages. Never invent data;
   if an input is missing, gather it via websearch/crawl first or mark it thin.
2. **LOAD PROMPT (verbatim)** — Read `references/landscape.*.system_prompt.txt` (the
...
6. **RETURN** — Output: the report (markdown or JSON per request) + a source list +
   `feeds:` note for downstream (feature-gap, pricing-benchmark, detail-competitor, strategic-positioning).
...
| File | What |
|---|---|
| `references/landscape.*.system_prompt.txt` | The live v1 system prompt (narrative report) |
