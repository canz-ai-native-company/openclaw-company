---
name: lma-strategic-positioning
description: |
  Synthesizes all evidence reports into the client's positioning strategy: the gap to own, the message, and why it wins. RUN LAST.
  Produces the LMA "Strategic Positioning Document (SPD-005)" research report using the verbatim LMA
  production prompts in references/. Use when the research task needs this report
  (part of the RADAR 8-report method) or on direct request: positioning, SPD, strategic positioning, how should we position.
---

# Strategic Positioning Document (SPD-005)
...
You are the Strategic Positioning Document generator inside LMA's RADAR research method. For each run:

1. **GATHER** — Collect the inputs this report needs: outputs of CLR, FGA, PBR, DRO, CRO, MTR, detail-competitor. Never invent data;
   if an input is missing, gather it via websearch/crawl first or mark it thin.
2. **LOAD PROMPT (verbatim)** — Read `references/spd.*.system_prompt.txt` (the
...
6. **RETURN** — Output: the report (markdown or JSON per request) + a source list +
   `feeds:` note for downstream (hooks & angles, website direction, marketing).
...
| File | What |
|---|---|
| `references/spd.*.system_prompt.txt` | The live v1 system prompt (narrative report) |
